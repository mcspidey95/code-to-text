...
function ThemeSelector({theme, setTheme} : ThemeSelectorProps){
    ...
    return (
        ...
        <div className="theme-selector" onClick={toggleDropdown}>
            ...
            {showDropdown && <div className="dropdown-menu relative top-[90px] w-[120px]">{themes.map((theme, i) => {
                return <button key={i} onClick={() => handleThemeChange(theme)} className="capitalize text-left">{theme}</button>
            })}</div>}
        </div>
        </OutsideClickHandler>
    )
